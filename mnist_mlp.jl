...
# ╔═╡ d16778d0-4dd8-11f1-ae53-592f5b8cd1f1
using MLDatasets:MNIST, ImageShow, ImageIO

# ╔═╡ 27c670da-b709-4543-b8dd-f80508a9f375
using Flux

# ╔═╡ abb4e008-da82-498e-bde7-0a1715cead26
X_train = MNIST(:train).features[:,:,1:2000]

# ╔═╡ 28f5bf10-8251-490c-a272-cee0735ad887
y_train = MNIST(:train).targets[1:2000]

# ╔═╡ a067bc17-5564-40f9-a41a-ba820cbbe65f
x1 = X_train[:,:,1]

# ╔═╡ 7633ac4e-ba9d-4109-b031-521ddb601e17
convert2image(MNIST, x1)

# ╔═╡ fead24fc-3953-4e0a-b384-f85e2ee8bdd5
[convert2image(MNIST, X_train[:,:,i]) for i=1:5]

# ╔═╡ bd935a9f-37a8-44c6-8e5f-6b246d5dc10f
x1[:]

# ╔═╡ 0ff7400f-6b1a-411a-be33-0fc8ef5f66a0
layer = Dense(28*28 => 10, tanh)

# ╔═╡ 08173c12-0d6d-4716-a7da-3ebf0ac8faa8
layer(x1[:])

# ╔═╡ caeeb641-f781-4e9f-a4f5-0f27ec4f6140
model= Chain(
	Dense(28*28=>32, tanh),
	Dense(32=>10, relu),
	softmax
...
# ╔═╡ 1ee47fbb-329b-4d78-8c65-fe14c98b724c
ŷ1 = model(x1[:])

# ╔═╡ 00ab5e86-8eb7-433d-a6e9-3056067744e2
y1 = y_train[1]

# ╔═╡ ee63aec1-4375-462e-83f0-b617b34e3522
Flux.onehot(y_train[1],0:9)

# ╔═╡ 55a80cee-edc5-46a5-ad53-d0a992e9fc8e
Flux.crossentropy(ŷ1,y1)

# ╔═╡ 7820c4f9-149f-4ca3-8b3e-777883e328f1
Flux.crossentropy(model(X_train[:,:,2][:]),Flux.onehot(y_train[2],0:9))

# ╔═╡ c329deab-45c4-4b6c-9a5c-0c58f377f70a
X1 = zeros(Float32, 784,8)

# ╔═╡ 6dd61773-0809-46a6-a143-6f7b254ae297
for i = 1:8
	X1[:,i] = X_train[:,:,i][:]
end

# ╔═╡ b21693c6-10d7-495a-99af-caaafb2a1de5
X1

# ╔═╡ b6462dc3-74fc-4d5a-a226-d230b8d81571
Ŷ1 = model(X1)

# ╔═╡ 939f3a7c-65f3-4d1f-a695-b0ade10bcffd
Y1 = Flux.onehotbatch(y_train[1:8],0:9)

# ╔═╡ 9dc886ed-4eb1-4e6b-a807-52a21af4c82b
Flux.crossentropy(Ŷ1,Y1)

# ╔═╡ 5f8b2cc3-ad77-403c-aeb8-89b6659d5dfd
z = Flux.stack([model(X_train[:,:,i][:]) for i =1:2000])

# ╔═╡ 429c2286-2307-403f-aef1-19da533eabbd
pred = Flux.onecold(z).-1

# ╔═╡ 1c25c083-2a51-4b89-86c0-07433b5eec0b
acc0 = sum(pred .== y_train)/length(y_train)

# ╔═╡ 952e5578-3521-43f6-8b40-3b52ecc83ae6
X= Flux.stack([X_train[:,:,i][:] for i =1:2000] )

# ╔═╡ 4894997c-2cb8-4e7b-a24b-43baab7895ed
y = Flux.onehotbatch(y_train,0:9)

# ╔═╡ 27d09906-0130-426c-baeb-76593673fa7e
data = [(X,y)]

# ╔═╡ 7b392d52-aae4-4408-b820-26290cb7e764
optim = Adam()

# ╔═╡ ead90c28-d548-4ac4-942d-acb357f3c8d8
state = Flux.setup(optim, model)

# ╔═╡ 6294665a-61a6-4fe9-a13b-c5de1bcbfb1c
cost(model,x, target) = Flux.crossentropy(model(x), target)

# ╔═╡ 2cffaf4e-f96a-4fb8-9b3f-56e58aef37ad
for i = 1:200
	Flux.train!(cost,model, data, state)
end

# ╔═╡ 0039643f-b56b-4759-84a2-83b0ec1c62d6
model2 = Chain(
	Dense(28*28,64,tanh),
	Dense(64, 32, relu),
	Dense(32,10),
...
# ╔═╡ 11e165d8-ffaa-453e-b90d-2f49ed8ff9c6
state2 = Flux.setup(optim, model2)

# ╔═╡ 84138041-fdda-4e91-808d-d8727df14f7a
for i = 1:10
	Flux.train!(cost,model2, data, state2)
end

# ╔═╡ 2464d235-a3f8-4004-bf2d-0d3f9d3d537c
y_test1 = Flux.onecold(model2(X)) .-1

# ╔═╡ 70746a41-1032-4732-86bf-ab59eecbf993
test= MNIST(:test)

# ╔═╡ 79c3e0fb-ae18-4944-8166-c6168b0120cb
X_test = test.features[:,:,1:2000]

# ╔═╡ ecaceccf-386f-485a-ad81-1f2786139ba1
#=╠═╡
sum(y_test .== y_train)/length(y_test)
  ╠═╡ =#

# ╔═╡ debebdab-66ec-4f3e-a82d-0051442b1a44
#=╠═╡
sum(y_test1 .== y_train)/length(y_test)
  ╠═╡ =#

# ╔═╡ 267502dd-7d3e-4fdc-b541-07f2193e8dd9


# ╔═╡ 7e4d648f-a0aa-482d-b09d-83821e1acad2
# ╠═╡ disabled = true
#=╠═╡
y_test = Flux.onecold(model(X)) .-1
  ╠═╡ =#

# ╔═╡ 40c2a95e-e0ee-4254-9629-2f9385514478
#=╠═╡
y_test = test.targets[1:2000]
  ╠═╡ =#
...
manifest_format = "2.0"
project_hash = "238713c175a49c799ca96c3458a13fc73b6be169"
